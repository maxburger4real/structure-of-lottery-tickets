...
def print_bias_pruned(model, layer=2):
    pruned_biases = list(np.array([b.item() for b in model.modules[layer].bias]) == 0)
    true_indices = [index for index, value in enumerate(pruned_biases) if value]
    print(true_indices)
...
def make_splitable_model(model, num_tasks, tile_offset=0.1):
    ...
    with torch.no_grad():
        for m in model.layers:
            ...
            tiling = (
                disconnected_weights((_out, _in), num_tasks).astype(np.float32) * 0.5
                + tile_offset
            )
